...
void produce(podcons_t *shared_data, size_t numberThreads) {
  ...
  // save size of struct to shared_data->size
  shared_data->size = tamanio;

  // /1/2/3/4/5/6/stop/stop/stop/
  for (size_t i = 0; i < numberThreads; i++) {
    // stop condition
    number_t *stop = NULL;
    // insert stop condition to queue
...
void *consume(void *data) {
  ...
  while (true) {
    ...
    if (value == prodcons->stopCondition) {
      ...
      goldbach(value);
      // put value into array of processed numbers
